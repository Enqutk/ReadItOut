...
bot.command('select_for_video', requireAdmin, async (ctx) => {
  ...
  if (!urlMatch) {
    return ctx.reply(
      'Usage: /select_for_video &lt;youtube_url&gt; &lt;id1&gt; [id2] [id3] ...\n\n' +
        'Example: /select_for_video https://youtu.be/abc123 def45678 ghi90123 jkl45678\n\n' +
        'Select 4–8 approved stories for a video. Fans get notified with the link.',
      { parse_mode: 'HTML' }
    );
  }
  ...
  if (ids.length < 4 || ids.length > 8) {
    return ctx.reply('❌ Select 4–8 stories.');
  }
