  ...
  return ctx.reply('👋 Welcome to Leyu & Mahi Bot!\n\nSubmit your fan stories – we\'ll read the best ones on our videos!', {
    ...
    reply_markup: {
      inline_keyboard: [
        ...
        [{ text: '📝 Submit via chat', callback_data: 'submit_via_chat' }],
      ],
    },
  });
});
...
bot.command('select_for_video', requireAdmin, async (ctx) => {
  ...
  if (!urlMatch) {
    return ctx.reply(
      'Usage: /select_for_video &lt;youtube_url&gt; &lt;id1&gt; [id2] [id3] ...\n\n' +
        'Example: /select_for_video https://youtu.be/abc123 def45678 ghi90123\n\n' +
        'Select approved stories for a video. Fans get notified with the link.',
      { parse_mode: 'HTML' }
    );
  }
  ...
  try {
    ...
    const selected = [];
    for (const id of ids) {
      const match = allApproved?.find((s) => s.id === id || s.id.startsWith(id));
      if (match) selected.push(match);
    }

    if (selected.length !== ids.length) {
      return ctx.reply(`❌ Could not find all stories. Found ${selected.length}/${ids.length}. Check IDs are approved.`);
    }
