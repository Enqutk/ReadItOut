...
const APP_URL_RAW = process.env.NEXT_PUBLIC_APP_URL || process.env.VERCEL_URL || 'read-it-out-ten.vercel.app';
const APP_URL = APP_URL_RAW.startsWith('http') ? APP_URL_RAW : `https://${APP_URL_RAW}`;

const menuButtonOpen = { type: 'web_app', text: 'Open', web_app: { url: APP_URL } };

async function setMenuButtonDefault() {
  if (!BOT_TOKEN || BOT_TOKEN === 'placeholder') return { ok: false, error: 'No BOT_TOKEN' };
  try {
    await bot.telegram.setChatMenuButton({ menuButton: menuButtonOpen });
    return { ok: true };
  } catch (e) {
...
bot.start(async (ctx) => {
  // Set Menu Button so "Open" appears in chat list (like BotFather). Not available in BotFather UI – only via API.
  try {
    await ctx.telegram.setChatMenuButton({ menuButton: menuButtonOpen }); // default for all users
    await ctx.telegram.setChatMenuButton({ chatId: ctx.chat.id, menuButton: menuButtonOpen });
  } catch (e) {
    console.warn('setChatMenuButton failed:', e?.message || e);
  }
...
module.exports = { bot, setMenuButtonDefault };
